...
def categorize_device_type(hostname):
    """Categorize device by hostname pattern.

    This matches HexTracker's methodology which categorizes vulnerabilities
    by the device type they were found on, not the vulnerability family.

    Hostname patterns:
    - pan = Palo Alto firewalls (various naming conventions)
    - nswan/swan = Cisco switches
    - nrwan/rwan = Cisco routers
    - wwbr = Cisco WAN bridges
    """
    h = str(hostname).lower()
    # Palo Alto patterns - any hostname containing 'pan'
    if 'pan' in h:
        return 'Palo Alto'
    # Cisco patterns - switches, routers, WAN bridges
    elif 'swan' in h or 'rwan' in h or 'wwbr' in h:
        return 'Cisco'
    else:
        return 'Other/General'
...
def analyze_data(df_prev, df_curr):
    ...
    # Add vendor column based on DEVICE TYPE (hostname pattern)
    # This matches HexTracker's methodology which categorizes by device, not vulnerability family
    df_prev['vendor'] = df_prev['asset.host_name'].apply(categorize_device_type)
    df_curr['vendor'] = df_curr['asset.host_name'].apply(categorize_device_type)
