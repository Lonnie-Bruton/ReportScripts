...
def analyze_data(df_prev, df_curr):
    ...
    # Add vendor column
    df_prev['vendor'] = df_prev['definition.family'].apply(normalize_vendor)
    df_curr['vendor'] = df_curr['definition.family'].apply(normalize_vendor)
...
def generate_report(results, output_path, prev_name, curr_name):
    ...
    rows = [
        ['Total Vulnerabilities', f"{overall['prev_total']:,}", f"{overall['curr_total']:,}", f"{overall['curr_total'] - overall['prev_total']:+,}"],
        ['Unique Assets', f"{overall['prev_assets']:,}", f"{overall['curr_assets']:,}", f"{overall['curr_assets'] - overall['prev_assets']:+,}"],
        ['Unique Definitions', f"{overall['prev_definitions']:,}", f"{overall['curr_definitions']:,}", f"{overall['curr_definitions'] - overall['prev_definitions']:+,}"],
    ]
    ...
    for sev in ['Critical', 'High', 'Medium', 'Low']:
        ...
        rows.append([sev, f"{data['prev']:,}", f"{data['curr']:,}", f"{data['change']:+,}"])
    add_styled_table(doc, headers, rows)
    ...
    for vendor, data in sorted(results['by_vendor'].items(), key=lambda x: x[1]['curr'], reverse=True):
        rows.append([vendor, f"{data['prev']:,}", f"{data['curr']:,}", f"{data['change']:+,}"])
    add_styled_table(doc, headers, rows)
    ...
    for vendor, sev_data in results['vendor_severity'].items():
        if sev_data:
            ...
            for sev in ['Critical', 'High', 'Medium', 'Low']:
                if sev in sev_data:
                    ...
                    rows.append([sev, f"{data['prev']:,}", f"{data['curr']:,}", f"{data['change']:+,}"])
            if rows:
                add_styled_table(doc, headers, rows, header_color="4472C4")
            doc.add_paragraph()
    ...
    for vendor in ['Cisco', 'Palo Alto']:
        ...
        if vendor_data['increases']:
            ...
            add_styled_table(doc, headers, rows, header_color=vendor_colors[vendor]['increase'], hyperlink_col=0)
        else:
        ...
        if vendor_data['decreases']:
            ...
            add_styled_table(doc, headers, rows, header_color=vendor_colors[vendor]['decrease'], hyperlink_col=0)
        else:
