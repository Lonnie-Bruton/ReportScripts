...
def categorize_device_type(hostname, family=None):
    ...
    Matches HexTracker's normalizeVendor() function:
    1. Check hostname patterns first (hostnameVendorPatterns config)
    2. Then check family patterns (familyVendorPatterns config)

    Hostname patterns (from HexTracker default config):
    - nfpan -> Palo Alto
    - n[rs]wan -> CISCO (matches nrwan OR nswan)

    Family patterns (fallback):
    - cisco -> Cisco
    - palo alto -> Palo Alto
    """
    ...
    # Step 1: Hostname patterns (HexTracker hostnameVendorPatterns)
    if h:
        # nfpan -> Palo Alto
        if 'nfpan' in h:
            return 'Palo Alto'
        # n[rs]wan -> Cisco (matches nrwan or nswan)
        if re.search(r'n[rs]wan', h):
            return 'Cisco'
